...
| T15 Dogfooding 评估增强 | 待 Review（修复后复核） | Claude | T14 | [65-t15-dogfooding-evaluation-start-package.md](65-t15-dogfooding-evaluation-start-package.md) | [69-t15-dogfooding-evaluation-result.md](69-t15-dogfooding-evaluation-result.md) | [70-t15-review-by-codex.md](70-t15-review-by-codex.md)（P1/P2/P3 已修复，待复核） | `npm run verify:dogfooding`（150/150）、`npm run check`（43 files）、`npm test`、`npm run verify:harness`（22/22）零回归 | 无 | Codex 复核 P1 修复（unresolved_count T9 口径）；T15 为 P1，不阻塞 T16。 |
