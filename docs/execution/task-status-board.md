...
| T13 页面级用户参与主界面最小实现 | 已完成（T13F 降级关闭） | Codex（T13F 功能骨架）；MiniMax 暂不可用 | T2，T3，T5，T6，T9，T12；T14 已完成可供复盘摘要消费 | 原启动包：[57-t13-user-participation-main-ui-start-package.md](57-t13-user-participation-main-ui-start-package.md)；降级启动包：[66-t13-functional-fallback-start-package.md](66-t13-functional-fallback-start-package.md) | [67-t13-functional-fallback-result.md](67-t13-functional-fallback-result.md)（branch: codex/t13f-t16-e2e） | [68-t13f-t16-review-by-claude.md](68-t13f-t16-review-by-claude.md)（通过；4 项观察不阻塞） | `npm run verify:page`（10/10）、`npm run verify:e2e`（15/15）、全量回归零失败 | A7（MiniMax 体验 Review）降级 | T13F 已关闭；完整 T13 仍需 MiniMax 恢复后补齐体验 Review。 |
