...
| T13 页面级用户参与主界面最小实现 | 待 Review | Codex（T13F 功能骨架）；MiniMax 暂不可用 | T2，T3，T5，T6，T9，T12；T14 已完成可供复盘摘要消费 | 原启动包：[57-t13-user-participation-main-ui-start-package.md](57-t13-user-participation-main-ui-start-package.md)；降级启动包：[66-t13-functional-fallback-start-package.md](66-t13-functional-fallback-start-package.md) | [67-t13-functional-fallback-result.md](67-t13-functional-fallback-result.md) | T13F Review 方：Claude；完整 T13 仍需 MiniMax 体验 Review | `npm run verify:page`、`npm run check`、`npm test`、T8/T9/T11/T12/T14/T3/T6/T7 验证通过；浏览器桌面/移动端通过 | A7 未关闭：MiniMax 页面体验 Review 未完成 | 等待 Claude Review；T13F 通过后只能解除功能骨架风险，不能关闭 A7。 |
...
| T16 首个端到端样例验证 | 待 Review | Codex | T1-T14；T15 可选；开发冲刺期间 MiniMax 不阻塞启动 | [71-t16-e2e-validation-start-package.md](71-t16-e2e-validation-start-package.md) | [72-t16-e2e-validation-result.md](72-t16-e2e-validation-result.md) | Claude | `npm run verify:e2e`、`npm run verify:page`、`npm run check`、`npm test`、T8/T9/T11/T12/T14/T3/T6/T7 验证通过；浏览器页面检查通过 | MiniMax 体验 Review 作为降级风险记录；A7 未关闭 | 等待 Claude Review；通过后可作为首个功能/数据层 E2E 收口证据。 |
