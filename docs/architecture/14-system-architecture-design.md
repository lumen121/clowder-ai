...
Clowder AI 首版架构以 Harness 作为治理核心，围绕本机、单用户、单项目、三 Agent 协作工作流构建。系统不是三个独立聊天 Agent 的并列输出，而是一个受页面级用户参与入口、工作项状态机、A2A 协议、权限护栏、交叉 Review、质量门禁、人工升级和复盘记忆共同约束的协作系统。

首版架构目标是让用户通过页面录入工作项、查看状态、处理阻塞确认、查看 Review / 门禁 / 复盘结果，并让功能需求和 Bug 修复工作项从录入到交付全程可见、可追踪、可治理、可恢复。CLI 可以保留为零阶段或内部入口，但不能成为首版用户参与的唯一入口。
...
- 页面级用户参与入口是首版主入口，时间线只是页面级用户参与层中的核心视图之一。
...
```mermaid
flowchart TD
    User["用户"] --> Page["页面级用户参与层"]
    CLI["CLI（零阶段 / 内部入口）"] --> WorkItem["工作项控制器"]
    Page --> WorkItem
    WorkItem --> Chat["统一聊天室与时间线"]
...
1. 保证用户可以通过页面录入工作项、查看状态、处理阻塞确认，并查看 Review、门禁和复盘结果。
2. 保证工作项从录入到交付全程可见、可追踪、可恢复。
3. 防止 Agent 在需求不清、方案未评估、Review 缺失时直接执行。
4. 支持 Codex、Claude、MiniMax 在清晰任务边界下并行协作。
5. 通过 worktree 隔离降低并行开发时的文件覆盖风险。
6. 通过交叉 Review、质量门禁和人工升级控制交付风险。
7. 将 Dogfooding 复盘沉淀为本地记忆，并反馈到后续任务。
8. 优先保证可追踪性和治理能力，再逐步提高自动化程度。

## 主要组件

### 页面级用户参与层

页面级用户参与层是首版用户主入口。它负责把工作项录入、状态可见、阻塞确认、Review、门禁和复盘查看暴露给用户。CLI 可以保留为零阶段或内部入口，但页面不能退化为 CLI 输出的包装。
...
- 提供工作项详情与状态视图。
- 提供阻塞、分歧和风险确认入口。
- 提供 Review、质量门禁和复盘结果视图。
- 将用户确认结果回写为结构化状态，而不是只追加聊天文本。

### 统一聊天室与时间线

统一聊天室是页面级用户参与层中的核心视图，也是用户、Codex、Claude、MiniMax 以及 Harness 状态的共享可见空间。它展示：
...
时间线不只是聊天记录，还承担审计和协作追踪职责。所有关键状态变化都必须进入时间线，但时间线不是唯一用户可见表面；工作项详情、阻塞确认、Review、门禁和复盘结果必须能通过页面直接查看。

### 工作项控制器

工作项控制器负责维护工作项生命周期、状态机、任务拆解、阻塞项和交付状态。它是用户请求进入系统后的主流程入口，并为页面级用户参与层提供工作项详情和状态读取能力。
...
- 判断当前阶段是否允许进入下一阶段。
- 将阻塞、失败和升级事件暴露给 Harness、时间线和页面级用户参与层。
...
页面查询与视图投影负责把结构化工作项、A2A、Review、门禁、升级和复盘记录整理为页面可直接读取的视角。它不是新的源数据模型，也不要求现在引入复杂 CQRS 或事件溯源；它的职责是避免页面只能从原始消息流中拼接状态。
...
- 提供时间线查询。
- 提供当前阻塞项和待确认项查询。
- 提供当前负责人、依赖和 Review 方查询。
- 提供 Review、质量门禁和复盘摘要查询。
...
页面读取视角不是新的源实体，而是对现有结构化记录的查询投影。首版至少需要提供：

- 工作项摘要查询：类型、目标、当前阶段、当前负责人、最近一次关键结论。
- 工作项详情查询：范围、任务拆解、依赖、Review 方、阻塞项、待确认项。
- 时间线查询：用户消息、Agent 消息、A2A、状态变化、Review、门禁、交付和复盘事件。
- Review 结果汇总查询：当前结论、修改要求、争议和待确认项。
- 门禁结果汇总查询：当前门禁状态、失败原因、修复状态和剩余风险。
- 复盘结果摘要查询：复盘结论、改进建议和技术执行建议。
...
1. 用户通过页面提交功能需求。
2. 页面级用户参与层创建工作项，并展示初始状态。
3. Codex 初步识别工作项类型。
4. Claude 和 MiniMax 可提出类型质疑、范围问题或风险提示。
5. Harness 判断是否存在阻塞性歧义；如需用户确认，页面展示确认入口并回写结构化决定。
6. Codex 形成方案和任务拆解。
7. Claude 和 MiniMax 评估方案、风险、可测试性和任务边界。
8. Harness 检查开发门禁。
9. 任务按边界串行或并行执行，页面持续展示当前状态、负责人、依赖和关键结论。
10. 作者请求非作者 Review。
11. Review 结果在页面可见；通过后执行质量门禁。
12. 门禁结果在页面可见；通过后准备本地提交。
13. 推送 feature 分支。
14. 可选创建 GitHub PR。
15. 记录复盘记忆，并在页面展示复盘结论和改进建议。

### Bug 修复流程

1. 用户通过页面提交 Bug。
2. 页面级用户参与层创建工作项，并展示初始状态。
3. Codex 记录现象、影响范围和可用复现信息。
4. Claude 评估可能原因和验证路径。
5. MiniMax 在 UI、视觉或交互相关 Bug 中参与方案或 Review。
6. 如果复现信息不完整但可以基于假设推进，系统记录假设、风险和验证路径，并在页面展示关键假设。
7. 如果高风险 Bug 缺少验证路径，Harness 升级给用户，页面展示确认入口并回写结构化决定。
8. 方案和任务拆解通过评估后进入开发。
9. 作者完成修复后请求非作者 Review。
10. Review 和回归验证结果在页面可见。
11. 执行质量门禁，门禁通过后进入交付流程。
12. 记录复盘记忆，并在页面展示复盘结论和改进建议。
...
升级不是失败，而是治理系统在风险超出可自治范围时的正常出口。首版中，升级必须能通过页面展示给用户，并将用户确认、拒绝或补充信息回写为结构化状态。
...
| 页面级用户参与入口 | 页面级用户参与层、工作项录入入口、统一聊天室与时间线 |
| 页面级工作项详情与状态可见 | 页面查询与视图投影、工作项控制器、详情视图 |
| 页面级 Review / 门禁 / 复盘查看 | 页面查询与视图投影、Review 与质量门禁引擎、Dogfooding 评估器 |
